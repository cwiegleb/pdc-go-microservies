...
func GetsInvoiceHandler(w http.ResponseWriter, r *http.Request) {
	...
	db.Raw("select dealers.id as dealer_id, articles.id as article_id, articles.text as article_text, order_lines.price as price from dealers, orders, order_lines, articles where orders.id = order_lines.order_id and order_lines.article_id = articles.id and articles.dealer_id = dealers.id").Scan(&accountingResult)
