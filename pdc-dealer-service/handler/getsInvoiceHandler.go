...
func GetsInvoiceHandler(w http.ResponseWriter, r *http.Request) {
	...
	db.Raw("select dealers.id as dealer_id, order_lines.article_id as article_id, order_lines.price as price from dealers, orders, order_lines where orders.id = order_lines.order_id").Scan(&accountingResult)
