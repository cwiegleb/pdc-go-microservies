...
func GetInvoiceHandler(w http.ResponseWriter, r *http.Request) {
	...
	db.Raw("select dealers.id as dealer_id, order_lines.article_id as article_id, dealers.external_id as external_id, order_lines.price as price from dealers, orders, order_lines where orders.id = order_lines.order_id and order_lines.dealer_id = dealers.id and dealers.id = ? order by article_id", vars["id"]).Scan(&accountingResult)
